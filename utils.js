...
function insertActeur(connection, faker) {
    ...
    var sql = "INSERT INTO acteur (NOM_ACTEUR, PRENOM_ACTEUR,NATION_ACTEUR,DATE_DE_NAISSANCE) VALUES ?";
    var values = [];

    for (var i = 0; i < 10; i++) {
        values.push([faker.name.lastName(), faker.name.firstName(), faker.address.country(), faker.date.past()]);
    }
...
function insertRole(connection) {
    ...
    var values = [];
    // Role cinema
    var role = ["Protagoniste", "Antagoniste", "Figurant", "Second rôle", "Cameo", "Voix Off"];
...
function insertRealisateur(connection, faker) {
    ...
    var sql = "INSERT INTO realisateur (NOM_REALISATEUR, PRENOM_REALISATEUR,NATION_REALISATEUR) VALUES ?";
    var values = [];

    for (var i = 0; i < 10; i++) {
        values.push([faker.name.lastName(), faker.name.firstName(), faker.address.country()]);
    }
...
function insertFilm(connection, faker) {
    ...
    var movieName = ["The Shawshank Redemption", "The Godfather", "The Godfather: Part II", "The Dark", "12 Angry", "Schindler's List", "The Lord of the Rings: The Return of the King", "Pulp Fiction", "The Good, the Bad and the Ugly", "The Lord of the Rings: The Fellowship of the Ring"];
    var movieLength = [142, 175, 202, 152, 96, 195, 201, 154, 161, 178];
    var movieGenre = ["Drama", "Crime", "Drama", "Crime", "Drama", "Biography", "Adventure", "Crime", "Western", "Adventure"];
    var movieReleaseDate = ["1994-09-23", "1972-03-14", "1974-12-20", "2008-09-18", "1957-04-10", "1993-11-30", "2003-12-01", "1994-10-14", "1966-12-23", "2001-12-19"];
    var sql = "INSERT INTO film (TITRE_FILM, DATE_DE_SORTIE,DUREE,GENRE,REALISER) VALUES ?";
    var values = [];

    for (var i = 0; i < 10; i++) {
        values.push([movieName[i], movieReleaseDate[i], movieLength[i], movieGenre[i], i + 1]);
    }
...
function insertseance(connection, faker) {
    ...
    var horaire = ["08:00:00", "09:00:00", "10:00:00", "12:00:00", "14:00:00", "16:00:00", "18:00:00", "20:00:00", "22:00:00", "00:00:00"];
    var sql = "INSERT INTO seance (DATE_SEANCE,HORAIRE,PROJETER,A_POUR_CATEGORIE) VALUES ?";
    var values = [];
...
function insertcategoriePlace(connection) {
    ...
    var values = [];
    var categorie = ["Normale", "Enfant", "Senior", "Etudiant", "Handicapé", "VIP"];
...
function insertTarif(connection) {
    ...
    var sql = "INSERT INTO tarif (TARIF,CATEGORIE_DE_LA_SEANCE,CATEGORIE_DE_LA_PLACE) VALUES ?";
    var values = [];
    for (var i = 1; i < 7; i++) {
        for (var j = 1; j < 7; j++) {
            values.push([i + j, i, j]);
...
function insertReservation(connection,faker) {
    ...
    var sql = "INSERT INTO reservation (NUMERO_SEANCE,NOM_SPECTATEUR,NUMERO_PLACE) VALUES ?";
    var values = [];
    var length = 100;
    ...
    for (var i = 1; i < length; i++) {
        ...
        values.push([random, faker.name.findName(), random2]);
    }
...
function resetIncrement(connection, table) {
    connection.query("ALTER TABLE " + table + " AUTO_INCREMENT = 1", function (err, result) {
        if (err) throw err;
